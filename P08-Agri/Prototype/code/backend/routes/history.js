...
async function get_latest_diagnosis_for_user(user_id) {
  if (!user_id) {
    return null
  }

  const latest_diagnosis = await Diagnosis.findOne({ user_id: user_id })
    .sort({ createdAt: -1, _id: -1 })
    .lean()

  return latest_diagnosis || null
}
...
router.post('/', requireAuth, async function (request, response) {
  try {
    ...
    let diagnosis_payload = request_body.diagnosis || null
    ...
    if (!diagnosis_payload) {
      diagnosis_payload = await get_latest_diagnosis_for_user(request.auth.userId)
    }
    ...
    response.json({
      ...
      diagnosis_used: diagnosis_payload
        ? {
            diagnosis: diagnosis_payload.diagnosis || null,
            confidence: diagnosis_payload.confidence,
            recommendations: Array.isArray(diagnosis_payload.recommendations) ? diagnosis_payload.recommendations : [],
            alternatives: Array.isArray(diagnosis_payload.alternatives) ? diagnosis_payload.alternatives : []
          }
        : null,
